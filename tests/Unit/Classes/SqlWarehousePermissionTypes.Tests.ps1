<#
    .SYNOPSIS
        Unit test for SqlWarehousePermissionTypes class (SqlWarehouseAccessControlEntry).
#>
...
Describe 'SqlWarehouseAccessControlEntry' -Tag 'SqlWarehouseAccessControlEntry' {
    Context 'When instantiating the class' {
        It 'Should not throw an error with mandatory property' {
            InModuleScope -ScriptBlock {
                { [SqlWarehouseAccessControlEntry] @{ PermissionLevel = 'CAN_USE' } } | Should -Not -Throw
            }
        }

        It 'Should be of the correct type' {
            InModuleScope -ScriptBlock {
                $aclInstance = [SqlWarehouseAccessControlEntry] @{
                    PermissionLevel = 'CAN_USE'
                    UserName        = 'user@example.com'
                }

                $aclInstance | Should -Not -BeNullOrEmpty
                $aclInstance.GetType().Name | Should -Be 'SqlWarehouseAccessControlEntry'
            }
        }

        It 'Should accept CAN_MANAGE permission level' {
            InModuleScope -ScriptBlock {
                { [SqlWarehouseAccessControlEntry] @{ PermissionLevel = 'CAN_MANAGE' } } | Should -Not -Throw
            }
        }

        It 'Should accept CAN_MONITOR permission level' {
            InModuleScope -ScriptBlock {
                { [SqlWarehouseAccessControlEntry] @{ PermissionLevel = 'CAN_MONITOR' } } | Should -Not -Throw
            }
        }

        It 'Should accept CAN_VIEW permission level' {
            InModuleScope -ScriptBlock {
                { [SqlWarehouseAccessControlEntry] @{ PermissionLevel = 'CAN_VIEW' } } | Should -Not -Throw
            }
        }

        It 'Should accept IS_OWNER permission level' {
            InModuleScope -ScriptBlock {
                { [SqlWarehouseAccessControlEntry] @{ PermissionLevel = 'IS_OWNER' } } | Should -Not -Throw
            }
        }
    }
    ...
    Context 'When setting and reading values' {
        Context 'When setting UserName principal' {
            It 'Should be able to set and read UserName' {
                InModuleScope -ScriptBlock {
                    $aclInstance = [SqlWarehouseAccessControlEntry]::new()
        ...
        Context 'When setting GroupName principal' {
            It 'Should be able to set and read GroupName' {
                InModuleScope -ScriptBlock {
                    ...
                    $aclInstance.PermissionLevel = 'CAN_MANAGE'

                    $aclInstance.GroupName | Should -Be 'data-engineers'
                    $aclInstance.PermissionLevel | Should -Be 'CAN_MANAGE'
                }
            }
        }
    ...
    Context 'When comparing two objects using method Equals()' {
        Context 'When both objects are equal' {
            It 'Should return $true for UserName principal' {
                InModuleScope -ScriptBlock {
                    $aclInstance1 = [SqlWarehouseAccessControlEntry] @{
                        UserName        = 'user@example.com'
                        PermissionLevel = 'CAN_USE'
                    }

                    $aclInstance2 = [SqlWarehouseAccessControlEntry] @{
                        UserName        = 'user@example.com'
                        PermissionLevel = 'CAN_USE'
                    }
    ...
    Context 'When using method ToString()' {
        ...
        It 'Should return a formatted string for GroupName principal' {
            InModuleScope -ScriptBlock {
                ...
                $aclInstance.ToString() | Should -Be 'Group: data-engineers - CAN_MANAGE'
            }
        }
        ...
        It 'Should return a formatted string with IS_OWNER permission level' {
            InModuleScope -ScriptBlock {
                $aclInstance = [SqlWarehouseAccessControlEntry] @{
                ...
                $aclInstance.ToString() | Should -Be 'User: admin@example.com - IS_OWNER'
            }
        }
